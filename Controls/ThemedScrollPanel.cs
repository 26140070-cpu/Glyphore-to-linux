...
internal sealed class ThemedScrollPanel : UserControl, IGlyphWheelScrollHost
{
    ...
    private bool _scrollBarAttached;
    ...
    public ThemedScrollPanel()
    {
        ...
        HandleCreated += (_, _) => AttachScrollBar();
        ...
        _viewport.KeyDown += HandleScrollKeyDown;
        _surface.KeyDown += HandleScrollKeyDown;
    }

    private void AttachScrollBar()
    {
        if (_scrollBarAttached) return;
        _scrollBarAttached = true;
    ...
    private void ApplyBackground()
    {
        ...
        if (_scrollBarAttached) _vertical.BackColor = BackColor;
    }
    ...
    private void HandleScrollKeyDown(object? sender, KeyEventArgs e)
    {
        if (!_vertical.Visible) return;
    ...
    {
        if (_updating || _content is null) return;
        if (_viewport.ClientSize.Width <= 0 || _viewport.ClientSize.Height <= 0)
        {
            // Algunos backends del emulador reportan ClientSize=0 en el primer layout;
            // sin reintento la barra nunca aparece (scrollbar "muerto").
            if (_layoutRetries < 8 && IsHandleCreated)
            {
                _layoutRetries++;
                try { BeginInvoke(new Action(() => { _layoutRetries = 0; RefreshScrollLayout(); })); } catch { }
            }
            return;
        }
        _layoutRetries = 0;
