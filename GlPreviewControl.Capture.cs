...
{
    private sealed record ExportCaptureRequest(
        GlyphoreScene? Scene,
        double TimeSeconds,
        bool IncludeRaster,
        RasterBackgroundMode BackgroundMode,
        Color? SolidColor,
        TaskCompletionSource<ExportFrame> Completion);

    private sealed record RasterCaptureRequest(
        GlyphoreScene Scene,
        double TimeSeconds,
        RasterBackgroundMode BackgroundMode,
        Color? SolidColor,
        byte[]? ReusableRgba,
        TaskCompletionSource<RasterFrame> Completion);

    public Task<ExportFrame> CaptureExportFrameAsync(
    ...
    {
        var tcs = new TaskCompletionSource<ExportFrame>(TaskCreationOptions.RunContinuationsAsynchronously);
        if (!_loaded)
        {
            tcs.TrySetException(new InvalidOperationException("OpenGL no disponible"));
            return tcs.Task;
        }
        lock (_captureQueueGate)
            _exportCaptureQueue.Enqueue(new ExportCaptureRequest(sceneSnapshot, timeSeconds, includeRaster, backgroundMode, solidColor, tcs));
        RequestNextFrameRendering();
        return tcs.Task;
    }

    public Task<RasterFrame> CaptureRasterExportFrameAsync(
        GlyphoreScene sceneSnapshot,
        double timeSeconds,
        RasterBackgroundMode backgroundMode,
        Color? solidColor = null,
    ...
    {
        var tcs = new TaskCompletionSource<RasterFrame>(TaskCreationOptions.RunContinuationsAsynchronously);
        if (!_loaded)
        {
            tcs.TrySetException(new InvalidOperationException("OpenGL no disponible"));
            return tcs.Task;
        ...
        lock (_captureQueueGate)
            _rasterCaptureQueue.Enqueue(new RasterCaptureRequest(sceneSnapshot, timeSeconds, backgroundMode, solidColor, reusableRgba, tcs));
        RequestNextFrameRendering();
        return tcs.Task;
    }
    ...
    public RasterFrame CaptureRasterExportFrame(
        ...
        Color? solidColor = null,
        byte[]? reusableRgba = null)
        => CaptureRasterExportFrameCore(sceneSnapshot, timeSeconds, backgroundMode, solidColor, reusableRgba);

    private RasterFrame CaptureRasterExportFrameCore(
        GlyphoreScene sceneSnapshot,
        double timeSeconds,
        RasterBackgroundMode backgroundMode,
        Color? solidColor,
