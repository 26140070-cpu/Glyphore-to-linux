...
Glyphoré is a procedural character art studio developed and officially supported for Windows. The Linux build now uses Majorsilence.Forms with its Avalonia backend and the shared scene/effect data model. It lets you create, customize, animate and export ASCII/Unicode scenes without a native Linux-specific UI bridge.

The live preview is rendered directly with cross-platform renderer, while exported character output remains real selectable and copyable text.

## Highlights

- C# / .NET 10 + WinForms
- Native cross-platform renderer 3.3 through Avalonia desktop backend
- renderer-resident real-time preview
- 52 effect families and 353 presets
- 22 color palettes and 27 character ramps
- Spanish and English interface
- Self-contained Windows x64 builds
- Native Linux x64 build (Majorsilence.Forms + Avalonia)
...
The cross-platform renderer preview uses a glyph atlas for performance, but actual text is reconstructed when copying or exporting ASCII/Unicode data.
...
The live preview is designed to remain renderer-resident. Procedural effects are evaluated on the GPU and rendered through the character atlas without copying every preview frame back to system memory. renderer readback is performed only when actual character data is required, such as when copying or exporting text.

The status bar displays a compact overview of the current effect, preset, ASCII resolution, target/real FPS, frame submission time and renderer. More detailed renderer information is available through its tooltip.
...
Install **.NET desktop development** and the **.NET 10 SDK** from Visual Studio Installer. Open `Glyphore.sln` and build in `Release`, or run `Build-Release.ps1`.

### Linux (native port)

Install the **.NET 10 SDK**, then from the `Glyphore` project directory run:

```bash
./build-linux.sh
```

The script restores dependencies and publishes a self-contained `linux-x64` single-file build to:
...
- Windows x64
- cross-platform renderer 3.3 capable desktop runtime
...
- Linux x64
- .NET 10 SDK for building (the publish script creates a self-contained runtime)
- cross-platform renderer 3.3 capable desktop runtime
- FFmpeg is optional and required only for video export profiles that use it

The Linux port uses Majorsilence.Forms with its Avalonia backend. The preview no longer requires a platform-specific native UI context.

## Troubleshooting

For a verbose Linux startup trace, use the bundled diagnostics launcher. It captures stdout/stderr and environment details in a timestamped log. When rebuilt with diagnostics enabled, managed startup phases and WinForms/Avalonia UI exceptions are also written to `%LOCALAPPDATA%/Glyphore/diagnostic.log` (or the application directory if that location is unavailable).
...
On Linux, include the distribution, desktop/session (X11 or Wayland), GPU model and driver, and whether the same scene/effect reproduces on Windows. The Linux port is native and does not require Wine.

## License

Glyphoré source code is licensed under the [MIT License](LICENSE).

Third-party component license texts remain embedded as resources and are also included in the repository. Those licenses apply only to the listed third-party components and do not replace Glyphoré's MIT license. The Linux port keeps the license catalog but does not currently expose the Windows About dialog.
...
La vista previa se renderiza directamente con cross-platform renderer, mientras que las exportaciones siguen siendo texto real, seleccionable y copiable.

## Características

- C# / .NET 10 + WinForms
- cross-platform renderer 3.3 nativo mediante Avalonia desktop backend
...
La preview cross-platform renderer utiliza un atlas de glifos para mantener el rendimiento, pero el texto real se reconstruye al copiar o exportar datos ASCII/Unicode.
...
La preview está diseñada para permanecer residente en renderer. Los efectos procedurales se calculan en la GPU y se renderizan mediante el atlas de caracteres sin copiar cada frame de preview de vuelta a la memoria del sistema. El readback solo se realiza cuando se necesitan los caracteres reales, como al copiar o exportar texto.

La barra de estado muestra de forma compacta el efecto y preset actuales, resolución ASCII, FPS objetivo/reales, tiempo de envío del frame y renderer. Los detalles completos del renderer están disponibles mediante su tooltip.
...
- Windows x64
- GPU y driver compatibles con cross-platform renderer 3.3

La build self-contained no requiere instalar .NET por separado. Windows es la plataforma principal y la referencia canónica.

### Linux

- Linux x64
- .NET 10 SDK para compilar (el script de publicación genera un runtime self-contained)
- GPU y driver compatibles con cross-platform renderer 3.3
- FFmpeg es opcional y solo hace falta para los perfiles de vídeo que lo utilizan

El port de Linux utiliza Majorsilence.Forms con backend Avalonia y conserva la arquitectura de controles tipo Forms sin depender de una capa nativa específica del escritorio.
...
En Linux, incluye también la distribución, sesión/backend gráfico (X11/Wayland), modelo y driver de GPU, y si la misma escena/efecto se reproduce en Windows. El port de Linux es nativo y no requiere Wine.
