...
Glyphoré is a GPU-accelerated procedural character art studio developed and officially supported for Windows. This repository also contains a native Linux x64 port built with GTKSystem.Windows.Forms, GtkSharp and OpenGL. It lets you create, customize, animate and export ASCII/Unicode scenes without having to program the effects yourself. Windows remains the canonical feature reference; the Linux port targets functional parity with the same scene/effect/shader data model.
...
- Native Linux x64 build (GTKSystem.Windows.Forms + Gtk.GLArea + OpenGL 3.3)
...
- Linux x64
- GTK3 + libepoxy runtime (for the native GTK UI and `Gtk.GLArea`)
...
The native Linux port uses GTKSystem.Windows.Forms for the WinForms-shaped UI and a native GTK3 `Gtk.GLArea` for OpenGL 3.3. It does not depend on legacy XAML UI layer or WGL. Linux desktop environments provide the GTK display backend (X11/Wayland) beneath the application.
...
Glyphoré es un estudio de arte procedural con caracteres acelerado por GPU desarrollado y soportado oficialmente para Windows. Este repositorio también incluye un port nativo para Linux x64 basado en GTKSystem.Windows.Forms, GtkSharp y OpenGL 3.3. Permite crear, modificar, animar y exportar escenas ASCII/Unicode sin tener que programar los efectos manualmente. Windows sigue siendo la referencia canónica de características; el port de Linux busca la paridad funcional con el mismo modelo de escenas, efectos, shaders y datos.
...
- Build nativa para Linux x64 (GTKSystem.Windows.Forms + Gtk.GLArea + OpenGL 3.3)
...
El port nativo de Linux utiliza GTKSystem.Windows.Forms, una capa de interfaz basada en GTK3 con API de WinForms, y `Gtk.GLArea` para OpenGL 3.3. No utiliza legacy XAML UI layer, WGL, Wine ni las APIs exclusivas de Windows `user32`/`gdi32`.
