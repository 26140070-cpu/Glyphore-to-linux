...
internal sealed partial class MainForm
{
    ...
    {
        Color picked = PickPaletteColor(ColorUtil.ParseHtmlOrWhite(_settings.PaletteStops[index]));
        if (picked.IsEmpty) return;
        EnsureCustomPalette();
        _settings.PaletteStops[index] = ColorTranslator.ToHtml(picked);
    ...
    {
        Color added = PickPaletteColor(Color.White);
        if (added.IsEmpty) return;
        EnsureCustomPalette();
        _settings.PaletteStops.Add(ColorTranslator.ToHtml(added));
    ...
    private static Color PickPaletteColor(Color initial)
    {
        if (OperatingSystem.IsWindows())
        {
            using var dialog = new ColorDialog { Color = initial, FullOpen = true };
            return dialog.ShowDialog() == DialogResult.OK ? dialog.Color : Color.Empty;
        }
        using var picker = new SimpleColorPickerForm(initial);
        return picker.ShowDialog() == DialogResult.OK ? picker.SelectedColor : Color.Empty;
    }
